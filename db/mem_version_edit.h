...
class MemVersionEdit {
 public:
  ...
  /*
  void SetFlushPointer(int level, const InternalKey& key) {
    flush_pointers_.push_back(std::make_pair(level, key));
  }
  */
  
  void SetFlushPointer( const InternalKey& key) {
    flush_pointers_ = key;
  }
 ...
 private:
  ...
  //std::vector<std::pair<int, InternalKey>>  flush_pointers_;
  InternalKey  flush_pointers_;

...
void DBImpl::RemoveObsoleteTables() {
  ...
  m_versions_->AddLiveTables(&live);
  //remove tables not in the m_versions
  std::map<uint64_t, MemTable*>::iterator iter;
  ...
  for (set_it = obsolete.begin(); set_it != obsolete.end(); set_it++) {
    all_memtables_.erase(*set_it);//remove from map
  }
}
...
void DBImpl::RecordMemTable(MemTable* mem, MemVersionEdit* edit) {
  ...
  all_memtables_.insert(std::make_pair(meta.number, mem));
}
...
Status DBImpl::WriteLevel0Table(MemTable* mem, VersionEdit* edit,
  ...
  if (s.ok() && meta.file_size > 0) {
    ...
    //测试fillseq
        ...
        //  if(level == 0) group = base->PickGroupForMemTableOutput(min_user_key, max_user_key);
    
     //测试fillrandom
     group = base->PickGroupForMemTableOutput(min_user_key, max_user_key);

    }
    edit->AddFile(level,group, meta.number, meta.file_size, meta.smallest,
                  meta.largest);
  }
...
void DBImpl::DiskFlush() {
  ...
  base_mem->Ref();
  MemTableMetaData* flushTable = m_versions_->PickFlush();//get flush table
  int level = base_mem->BaseLevel();
  ...
  if (s.ok()) {
    //remove memtable from m_versions
    MemVersionEdit mem_edit;
    mem_edit.RemoveTable(level, flushTable->number);
    m_versions_->Apply(&mem_edit);
    base_mem->Unref();
...
void DBImpl::MaybeScheduleCompaction() {
  ...
  if (background_compaction_scheduled_) {
    ...
    // Already got an error; no more changes
  } else if (!m_versions_->NeedsFlush() && manual_compaction_ == nullptr && !versions_->NeedsCompaction()) {
    // No work to be done
  } else {
...
void DBImpl::BackgroundCompaction(bool is_mem) {
  ...
  if (is_mem) {
    ...
    if (m_versions_->NeedsCompaction()) {
      bool has_lock = false;
      if (m_versions_->current()->NeedCompactWithLock()) {//if need sync with flush
        mutex_.Unlock();
        thread_mutex_.Lock();
        has_lock = true;
        mutex_.Lock();
      ...
      CompactionState* compact = new CompactionState(mc);
    ...
    if(versions_->NeedsCompaction()){
        ...
        if (c == nullptr) {
          ...
          c->edit()->AddFile(c->level() + 1, f->group_number, f->number ,f->file_size, f->smallest, f->largest);
...
void DBImpl::OpenCompactionOutputTable(CompactionState* compact) {
  ...
  uint64_t table_number;
    {
    mutex_.Lock();
    table_number = m_versions_->NewTableNumber();
    ...
    all_memtables_.insert(
        std::make_pair(table_number, out.mem_table));  //add mem to map
    compact->new_mem = out.mem_table;
    compact->has_entry = false;  //new mem has no entry
...
void DBImpl::InstallMemCompactionResults(CompactionState* compact) {
  ...
  compact->m_compaction->AddInputDeletions(
      compact->m_compaction->edit());  // edit delete table
  for (size_t i = 0; i < compact->outputs.size(); i++) {
    const CompactionState::Output& out = compact->outputs[i];
    compact->m_compaction->edit()->AddTable(level + 1, out.number,
                                            out.table_size, out.mem_table,
...
Status DBImpl::InstallCompactionResults(CompactionState* compact) {
  ...
  for (size_t i = 0; i < compact->outputs.size(); i++) {
    const CompactionState::Output& out = compact->outputs[i];
    compact->compaction->edit()->AddFile(level + 1, 0, out.number, out.file_size,
                                         out.smallest, out.largest);
  }
  ...
  while (input->Valid() && !shutting_down_.load(std::memory_order_acquire)) {
    ...
    if (!ParseInternalKey(key, &ikey)) {
      ...
      if (last_sequence_for_key <= compact->smallest_snapshot) {
        drop = true;
        //calculate drop kv size
        drop_info_.first++;
        drop_info_.second+=input->key().size()+VarintLength(input->key().size());
        drop_info_.second+=input->value().size()+VarintLength(input->value().size());
      }
    ...
    if (!drop) {
      ...
      compact->new_mem->CompactAdd(key, input->value());
...
Status DBImpl::MakeRoomForWrite(bool force) {
  ...
  Status s;
  while (true) {
    int max_group_number= versions_->current()->Max_Group();
    if (!bg_error_.ok()) {
      // Yield previous error
      s = bg_error_;
      break;
    }else if (allow_delay && m_versions_->NumLevelTables(0) >=
                                  config::kMemL0_SlowdownWritesTrigger) {
      mutex_.Unlock();
      //env_->SleepForMicroseconds(10);
      allow_delay = false;  // Do not delay a single write more than once
      mutex_.Lock();
    } else if (allow_delay && max_group_number >= config::kL0_SlowdownWritesTrigger) {
    ...
    //测试fillseq,readrandom，将此处注释
    else if (m_versions_->NumLevelTables(0) >=
               config::kMemL0_StopWritesTrigger) {
      Log(options_.info_log, "Too many M1 tables; waiting...\n");
      background_mem_work_finished_signal_.Wait();
    }
  
    else if ( max_group_number >= config::kL0_StopWritesTrigger) {
      // There are too many level-0 files.
      Log(options_.info_log, "Too many L0 group ; waiting...\n");
      std::fprintf(stdout, " group:%d,sst_number:%d; waiting...\n",max_group_number,versions_->NumLevelFiles(0));
      background_work_finished_signal_.Wait();
    }  

...
// m0-m2 size
static double MaxBytesForMemLevel(const Options* options, int level) {
  //double result = 5 * 4 * 1024 * 1024.0;  //m1层只有5个表
  
   double result =  4 * options->max_table_size;//m0:2
   if(level > 0){
     result = 5 * options->max_table_size;//m1:4
     while(level > 1){//m2:20
      result *= 10;
      level--;
     }
...
Status MemVersion::Get(const LookupKey& k, std::string* value) {
  struct State {
    ...
    static bool Match(void* arg,MemTableMetaData* f) {
      ...
      bool is_found = m->Get(*(state->lookKey), state->value, &state->s);
      if (is_found) {
        state->found = true;
        return false;
      } else {
...
class MemVersionSet::Builder {
 ...
 public:
  ...
  void Apply(MemVersionEdit* edit) {
    ...
    //update flush pointers
    /*
    for (size_t i = 0; i < edit->flush_pointers_.size(); i++) {
      const int level = edit->flush_pointers_[i].first;
      vset_->flush_pointer_ =
          edit->flush_pointers_[i].second.Encode().ToString();
    }
    */
    vset_->flush_pointer_ = edit->flush_pointers_.Encode().ToString();
    ...
    for (const auto& deleted_table_set_kvp : edit->deleted_tables_) {
      ...
      levels_[level].deleted_tables.insert(number);
    }
    // Add new tables
  ...
  void SaveTo(MemVersion* v) {
    ...
    for (int level = 0; level < config::kNumMemLevels; level++) {
      ...
      std::vector<MemTableMetaData*>::const_iterator base_end =
          base_tables.end();
      const TableSet* added_tables =
          levels_[level].added_tables;  //这层新加入的表
...
void MemVersionSet::AppendVersion(MemVersion* v) {
  ...
  current_ = v;//set current version
  v->Ref();
...
void MemVersionSet::Apply(MemVersionEdit* edit) {
  ...
  MemVersion* v = new MemVersion(this);
  {
    ...
    builder.Apply(edit);
    builder.SaveTo(v);  //save to a new version
  }
  Finalize(v);
  AppendVersion(v);
}
...
void MemVersionSet::AddLiveTables(
    std::set<uint64_t>* live) {  //把所有version中的表都添加到live
...
Iterator* MemVersionSet::MakeInputIterator(MemCompaction* c) {
  ...
  for (int which = 0; which < 2; which++) {
    if (!c->inputs_[which].empty()) {
      if (c->level() + which == 0) {
        //m0
        const std::vector<MemTableMetaData*>& tables = c->inputs_[which];
        for (size_t i = 0; i <tables.size(); i++) {
        ...
        list[num++] = NewMemTwoLevelIterator(
            new MemVersion::LevelTableNumIterator(icmp_, &c->inputs_[which]));
      }
    }
  }
...
void MemVersionSet::Finalize(MemVersion* v) {
  ...
  int best_level = -1;
  double best_score = -1;
  uint64_t all_mem_bytes = 0;

  for (int level = 0; level < config::kNumMemLevels-1; level++) {
    double score;
    const uint64_t level_bytes = TotalTableSize(v->tables_[level]);//根据总容量
    all_mem_bytes += level_bytes;
    score = static_cast<double>(level_bytes) / MaxBytesForMemLevel(options_,level); 
    if (score > best_score) {
      best_level = level;
      best_score = score;
    }
  ...
  all_mem_bytes += TotalTableSize(v->tables_[config::kNumMemLevels-1]);//加上最后一层
  //达到write_buffer_size，flush
  //std::fprintf(stdout,"current memory size MB: %lu \n",all_mem_bytes/(1024*1024));
  v->flush_score_= 1.0 *static_cast<double>(all_mem_bytes) / options_->write_buffer_size;
}


MemTableMetaData* MemVersionSet::PickFlush() {//轮讯选择一个sst
  //std:fprintf(stdout,"disk flush，m2的sst个数：%d",current_->tables_[config::kNumMemLevels-1].size());
  int level = current_->BaseLevel();

  MemTableMetaData* meta = current_->tables_[level][0];
  // Pick the first file that comes after flush_pointer_
    for (size_t i = 0; i < current_->tables_[level].size(); i++) {
      MemTableMetaData* f = current_->tables_[level][i];
      if ( flush_pointer_.empty() ||
          icmp_.Compare(f->largest.Encode(), flush_pointer_) > 0) {
       meta = f;
        break;
      }
    }
    if(!meta) meta = current_->tables_[level][0];
  return meta;
}
...
MemCompaction* MemVersionSet::PickCompaction() {
  ...
  if (size_compaction) {
    ...
    assert(level >= 0);
    assert(level + 1 < config::kNumMemLevels);
    MemCompaction* c = new MemCompaction(options_,level);
    ...
    if (c->inputs_[0].empty()) {
      c->inputs_[0].push_back(current_->tables_[level][0]);
    }
...
//——————————————————————————————————————————————————
// memcompaction
MemCompaction::MemCompaction(const Options* options,int level)
    : level_(level),
...
void MemCompaction::AddInputDeletions(MemVersionEdit* edit) {
  for (int which = 0; which < 2; which++) {
    for (size_t i = 0; i < inputs_[which].size(); i++) {
      edit->RemoveTable(level_ + which, inputs_[which][i]->number);
    }
  }
}
